...
def plot_whisker_boxes(my_data,
    ...
    """
    # --- FIX 1: Correctly get the dimension from the data shape ---
    dim = my_data.shape[1]
    
    # horizontal whiskerboxes
    if rotate:
        # --- FIX 2: Plot the entire 'my_data' array, not a slice ---
        axis.boxplot(my_data, showmeans=False,
    ...
    # vertical whisker boxes
        ...
        # --- FIX 2 (cont.): Plot the entire 'my_data' array ---
        axis.boxplot(my_data, showmeans=False,
                   boxprops=dict(linewidth=lw, color='black'),
                   whiskerprops=dict(linestyle='-', linewidth=lw, color='black'),
                   medianprops=dict(linestyle='-', linewidth=lw, color='blue'),
        ...
        if theo is not None:
            for i_feature in range(dim):
                axis.plot(i_feature + 1,
                        theo[i_feature], # Indexing adjusted for no intercept
                        'x',
                        markersize=20,
                        markeredgewidth=5,
                        zorder=10,
