...
import os  # <<< CHANGE: Imported the os module

# Assuming these utility files are in the specified paths relative to your script
...
from utils.output import OUT_DIR, out_path
...
if __name__ == "__main__":
    ...
    for p in [4,5]:
        ...
        if not os.path.exists(results_dir):
            ...
            print(f"Created directory: {results_dir}") # Optional: confirmation message
            
        # Create the full path for the output file
        s_name = str(out_path("cancellation_{}_boxes".format(p)))
        
        # --- END: CORRECTED CODE ---
        
        # Save the figure to the specified path
        fig.savefig(s_name + '.pdf',format='pdf',bbox_inches = 'tight',pad_inches = 0)
        print(f"Figure saved to {s_name}.pdf") # Optional: confirmation message